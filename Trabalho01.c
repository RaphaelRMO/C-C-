...
void cebolinha (char nomearq[40]){

    FILE *fptr, *fcebolinha;
    int fechArquivo;
    char ch, chant2, chant;

    if ((fptr = fopen(nomearq,"r")) == NULL){
        printf("\n***ERRO, ARQUIVO NAO FOI ABERTO***\n\n");
        return;
    }if ((fcebolinha = fopen("cebolinha.txt","w")) == NULL){
        printf("\nErro na abertura do arquivo cebolinha.txt");
        return;
    }else{
        while ((ch=getc(fptr))!=EOF){
            //printf("%c",ch);
